		...
		if (!(sender instanceof Player)) {
			...
			if (args.length == 0) {
				if (Config.getChargeTimeBased()) {
					player.sendMessage("You have " + Vault.getTime(player)
							+ " of time left.");
					...
					if (!MagicCarpet.getCarpets().hasPaidFee(player)) {
						player.sendMessage("You need to pay a one time fee of "
								+ String.valueOf(Config.getChargeAmount())
								+ " "
				...
				if (Config.getChargeTimeBased()) {
					...
					for (Entry<String, TimePackage> set : Vault.getPackages()
							.entrySet()) {
						TimePackage tp = set.getValue();
						player.sendMessage("Plan '" + set.getKey() + "' gives "
								+ Vault.getTime(tp.getTime()) + " and costs "
				...
				if (Config.getChargeTimeBased()) {
					if (Vault.getPackage(MagicCarpet.getCarpets()
							.getAutoPackage(player)) == null) {
